...
def simulate_sky_source(lmn_coord: np.array, baselines: np.array, freq: float, width_rad: float = 0): #width_rad size of diffuse point
    """
    ...
    """
    # uses geometry to measure delay (phase) according to space object position in the sky and distance between antennas
    #pointlike_phase = np.exp(2j * np.pi * freq * baselines.dot(np.array(lmn_coord)) / SPEED_OF_LIGHT)
    
    #b_lengths = np.linalg.norm(baselines, axis=-1) 
    
    # Gauss bell expansion
    #expansion = np.exp(- (np.pi * width_rad * b_lengths * freq / SPEED_OF_LIGHT)**2 / (4 * np.log(2)))
    
    return np.exp(2j * np.pi * freq * baselines.dot(np.array(lmn_coord)) / SPEED_OF_LIGHT) # model for bright sources
    #return pointlike_phase * expansion
